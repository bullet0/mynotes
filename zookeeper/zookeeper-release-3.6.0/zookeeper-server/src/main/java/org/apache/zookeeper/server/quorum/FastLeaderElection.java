...
public class FastLeaderElection implements Election {
                    ...
                    try {
                        ...
                        if (!validVoter(response.sid)) {
                            ...
                            if (self.getPeerState() == QuorumPeer.ServerState.LOOKING) {
                                 ...
                                 */
                                // 如果该服务器已经有了角色，但是发送确认的服务器正在看，则将发送一下当前自己的状态。
                                Vote current = self.getCurrentVote();
                                if (ackstate == QuorumPeer.ServerState.LOOKING) {
                                    if (self.leader != null) {
    ...
    private void sendNotifications() {
        for (long sid : self.getCurrentAndNextConfigVoters()) {
            ...
            ToSend notmsg = new ToSend(
                ...
                sid,
                proposedEpoch,
                qv.toString().getBytes()); // 这里当前配置文件中集群的信息也会被广播出去
    ...
    protected SyncedLearnerTracker getVoteTracker(Map<Long, Vote> votes, Vote vote) {
        ...
        // 这里的self.getQuorumVerifier()和 self.getLastSeenQuorumVerifier()都是验证器
        // 之后循环的时候这两个作用一致
        voteSet.addQuorumVerifier(self.getQuorumVerifier()); // 将仲裁验证器加入 qvAcksetPairs 中
         ...
         */
        for (Map.Entry<Long, Vote> entry : votes.entrySet()) { // 遍历选票箱中选票
            if (vote.equals(entry.getValue())) { // 如果自己的提案与某个选票相同
                voteSet.addAck(entry.getKey()); // 就将这个选票的发起人sid加入到SyncedLearnerTracker中的qvAcksetPairs中的ackset中
                // 之后是否过半判断的就是这个 ackset 集合的长度,是否超过半数
            }
        }
    ...
    private void setPeerState(long proposedLeader, SyncedLearnerTracker voteSet) {
        ...
        self.setPeerState(ss); // 设置自己的角色
        if (ss == ServerState.LEADING) {
            leadingVoteSet = voteSet; //如果是领导者,还会保存一下当前选举的仲裁器
        }
    }
    ...
    public Vote lookForLeader() throws InterruptedException {
        ...
        try {
            ...
            synchronized (this) {
                ...
                // getInitId() 这个方法就是如果自己可以投票,就返回自己的sid,否则返回 Long.MIN_VALUE
                // 这也是为啥说上来先投自己一票,先认为自己是leader
                // 同时也说明了观察者不具备投票权
                // getPeerEpoch() 如果没有,会从磁盘中读
                // 这里构造提案的时候如果是observer会设置sid = Long.MIN_VALUE,所以O也参与投票,只是投出去别人不看
                // 既然能投票也就能接受投票
                updateProposal(getInitId(), getInitLastLoggedZxid(), getPeerEpoch()); // 更新提案
            }
            ...
            LOG.info(
                ...
                self.getId(),
                Long.toHexString(proposedZxid));
            sendNotifications(); // 把选票放入sendqueue队列中就完了,之前QuorumCnxManager对象中的发送线程会把他发出去
            ...
            while ((self.getPeerState() == ServerState.LOOKING) && (!stop)) {
                ...
                if (n == null) {
                ...
                } else if (validVoter(n.sid) && validVoter(n.leader)) { // 有选举消息被接受,要确认被通知的sid和要选举的sid在集群配置信息中
                    /*
                    ...
                    case LOOKING:
                        ...
                        if (n.electionEpoch > logicalclock.get()) {
                            ...
                            if (totalOrderPredicate(n.leader, n.zxid, n.peerEpoch, getInitId(), getInitLastLoggedZxid(), getPeerEpoch())) {
                                updateProposal(n.leader, n.zxid, n.peerEpoch); // observer的sid最小,啥都得听他们的
                            } else {
                        ...
                        if (voteSet.hasAllQuorums()) { //如果过半

                            // Verify if there is any change in the proposed leader
                            // 再从recvqueue中获取选票,并判断选票上的提案是否比自己牛逼,如果是,就把选票再放回队列中,break去下次循环,让上面代码处理更换提案的逻辑
                            ...
                            if (n == null) {//拿不到新的选票
                                ...
                                leaveInstance(endVote); // 这里传入endVote主要是为了debug日志,里面做的事就是清空recvqueue队列
                                return endVote; //返回这张具有历史意义的选票
                            }
                        }
                        break;
                    case OBSERVING:
                    ...
                    case FOLLOWING:
                    case LEADING:
                        // 发选票给我的竟然是LEADER/FOLLOWING角色,可能就是我挂了,人家都选完了
                        /*
