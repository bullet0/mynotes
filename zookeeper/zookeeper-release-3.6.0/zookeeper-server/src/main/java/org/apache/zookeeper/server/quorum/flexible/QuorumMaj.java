    ...
    public QuorumMaj(Properties props) throws ConfigException {
        for (Entry<Object, Object> entry : props.entrySet()) {
            ...
            if (key.startsWith("server.")) {
                ...
                // 根据集群创建一个新的服务节点信息QuorumServer,里面有自己的2个端口地址
                QuorumServer qs = new QuorumServer(sid, value);
                // 每个服务器都持有所有的集群信息 在 allMembers 中
                allMembers.put(Long.valueOf(sid), qs);
                if (qs.type == LearnerType.PARTICIPANT) { // 默认不设置就是 PARTICIPANT
                    votingMembers.put(Long.valueOf(sid), qs); // 可投票的节点
                } else {
                    observingMembers.put(Long.valueOf(sid), qs); //不可投票的节点
                }
