...
public class ZooKeeperMain {
    ...
    static class MyCommandOptions {
        ...
        public boolean parseOptions(String[] args) {
            ...
            while (it.hasNext()) {
                ...
                //在处理开头不是 - 的参数
                //进入这里就会返回true了,不会再迭代了,所以在内部做了迭代,继续将后面的参数
                //放入 cmdArgs ,也就是命令行传参的时候不能在 非 - 开头的参数后面再跟 - 开头的参数了
                if (!opt.startsWith("-")) {
                    command = opt;
    ...
    protected void connectToZK(String newHost) throws InterruptedException, IOException {
        ...
        //调用ZooKeeperAdmin构造器创建一个 ZooKeeper 对象
        //ZooKeeperAdmin啥也没干,直接将参数过度到了ZooKeeper类
    ...
    public static void main(String[] args) throws CliException, IOException, InterruptedException {
        ...
        // run 逻辑简单,与Thread无关
        // 程序正常运行, 命令行等待接收客户指令,发送给服务器去执行
        main.run();
    }

    public ZooKeeperMain(String[] args) throws IOException, InterruptedException {
        //将命令行传入的参数解析一下
        //-server   -timeout   -r 的参数 放入 Map<String, String> options
        //不是-开头的参数 放入 List<String> cmdArgs
        // cl封装了  options 和 cmdArgs
    ...
    void run() throws CliException, IOException, InterruptedException {
        if (cl.getCommand() == null) {
            ...
            try {
                ...
                // 上面基本都是 打印消息 ,这里会一直监听命令行的输入,然后执行命令
