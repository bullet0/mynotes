...
public class Observer extends Learner {
    ...
    void observeLeader() throws Exception {
        ...
        try {
            ...
            // 从选票currentVote中获取领导的sid
            // observer可以连接到任意Leader或者follower,但是需要设置nextLearnerMaster,否则默认是连接leader
            QuorumServer master = findLearnerMaster();
            try {
                // 连上领导地址
                // FOLLOWING 必须连接到领导, OBSERVING 只要连接上任意一个 LearnerMaster 即可
                connectToLeader(master.addr, master.hostname);
                connectTime = System.currentTimeMillis();
                // 一旦连接到领导者或学习者主机，执行握手协议以建立跟随/观察连接。
                long newLeaderZxid = registerWithLeader(Leader.OBSERVERINFO);
                if (self.isReconfigStateChange()) { // 查看 reconfigFlag 的状态,一般是false
    ...
    private QuorumServer findLearnerMaster() {
        ...
        final QuorumPeer.QuorumServer master = (prescribedLearnerMaster == null)
            ? self.findLearnerMaster(findLeader()) // 这里从currentVote中获取领导
    ...
    protected void processPacket(QuorumPacket qp) throws Exception {
        ...
        case Leader.SYNC:
            ...
            break;
        case Leader.INFORM: //obseerver主要就这步
