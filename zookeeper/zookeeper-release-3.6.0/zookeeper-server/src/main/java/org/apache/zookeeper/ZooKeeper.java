...
public class ZooKeeper implements AutoCloseable {
    ...
    public ZooKeeper(
        ...
        cnxn = createConnection(
            ...
            this,
            watchManager,
            // 这个方法通过反射创建了 ClientCnxnSocket 对象,默认使用的是ClientCnxnSocketNIO的实现
            // 作为底层传输层
            // 此处可以通过 设置 ZKClientConfig.ZOOKEEPER_CLIENT_CNXN_SOCKET(zookeeper.clientCnxnSocket) 参数为 "org.apache.zookeeper.ClientCnxnSocketNetty" 将底层链接转为netty
            getClientCnxnSocket(),
            canBeReadOnly);
        cnxn.start();
    }
    ...
    private static HostProvider createDefaultHostProvider(String connectString) {
        //用 ConnectStringParser 个对象 将 字符串的地址,解析成 addr , port  List<InetSocketAddress>
