...
public class ClientCnxnSocketNIO extends ClientCnxnSocket {
    ...
    void doIO(Queue<Packet> pendingQueue, ClientCnxn cnxn) throws InterruptedException, IOException {
        ...
        // 如果写就绪,就把数据写入packet的bytebuffer,
        // 然后将packet从outgoingQueue移除,放入pendingQueue中
    ...
    void registerAndConnect(SocketChannel sock, InetSocketAddress addr) throws IOException {
        ...
        // 立即返回结果,不论连上或者连不上
        // 方法上的注释
        // 如果此通道处于非阻塞模式，则此方法的调用将启动非阻塞连接操作。如果像本地连接那样立即建立连接，
        // 则此方法返回true。否则，此方法返回false，
    ...
    void doTransport(
        ...
        for (SelectionKey k : selected) {
            ...
            if ((k.readyOps() & SelectionKey.OP_CONNECT) != 0) {
                // 因为之前是立即连接,所以不确保已经正确连接
                // 这里调用finishConnection再确认一遍
                //  通过将套接字通道设置为非阻塞模式，然后调用其connect方法，可以启动非阻塞连接操作。一旦建立连接或尝试失败，
                //  套接字通道将变为可连接状态，并且可以调用此方法来完成连接序列。如果连接操作失败，
                //  则调用此方法将导致引发适当的IOException。
                //  如果已经连接了此通道，则此方法将不会阻塞，并将立即返回true ,所以不必担心重复调用的问题
                if (sc.finishConnect()) {
                    updateLastSendAndHeard();
                    updateSocketAddresses();
                    sendThread.primeConnection(); // 这个方法总是要调用一次的,所以第一次连接总会发送一个包
                }
            } else if ((k.readyOps() & (SelectionKey.OP_READ | SelectionKey.OP_WRITE)) != 0) {
                // 读写操作,这里传入的 pendingQueue 是个 ArrayDeque
                doIO(pendingQueue, cnxn);
            }
        }
        if (sendThread.getZkState().isConnected()) {
            // findSendablePacket(outgoingQueue, sendThread.tunnelAuthInProgress())
            // 这一步相当于找到一个可发送的数据包放到队列头上
            if (findSendablePacket(outgoingQueue, sendThread.tunnelAuthInProgress()) != null) {
                enableWrite();
            }
        }
